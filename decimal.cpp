...
int decimal_actual_fraction(const decimal_t *from) {
    ...
    dec1 *buf0 = (dec1 *)(from->buf + ROUND_UP(from->intg) + ROUND_UP(frac) - 1);
...
int string2decimal(const char *from, decimal_t *to, const char **end) {
    ...
    while (s < end_of_string && my_isspace( *s)) s++;
    ...
    s1 = s;
    while (s < end_of_string && isdigit(*s)) s++;
    intg = (int)(s - s1);
    if (s < end_of_string && *s == '.') {
        endp = s + 1;
        while (endp < end_of_string && isdigit( *endp))
            endp++;
        frac = (int)(endp - s - 1);
    } else {
        frac = 0;
    ...
    *end = endp;
    ...
    if (endp + 1 < end_of_string && (*endp == 'e' || *endp == 'E')) {
        int str_error;
        long long exponent = my_strtoll10(endp + 1, &end_of_string, &str_error);
...
static int ull2dec(ulong long from, decimal_t *to) {
    int intg1;
    int error = E_DEC_OK;
    ulong long x = from;
    dec1 *buf;
...
int ulonglong2decimal(ulong long from, decimal_t *to) {
    to->sign = 0;
...
int longlong2decimal(long long from, decimal_t *to) {
    if ((to->sign = from < 0))
        return ull2dec(from == LLONG_MIN ? static_cast<ulong long>(from) : -from,
                       to);
    return ull2dec(from, to);
}

int decimal2ulonglong(decimal_t *from, ulong long *to) {
    dec1 *buf = from->buf;
    ulong long x = 0;
    int intg, frac;
    ...
    for (intg = from->intg; intg > 0; intg -= DIG_PER_DEC1) {
        ulong long y = x;
        x = x * DIG_BASE + *buf++;
        if ( (y > ((ulong long)ULLONG_MAX / DIG_BASE) || x < y)) {
            *to = ULLONG_MAX;
            return E_DEC_OVERFLOW;
        }
    }
    *to = x;
    for (frac = from->frac;  (frac > 0); frac -= DIG_PER_DEC1)
...
int decimal2longlong(decimal_t *from, long long *to) {
    ...
    for (intg = from->intg; intg > 0; intg -= DIG_PER_DEC1) {
        ...
        */
        ...
        x = x * DIG_BASE;
        const long long digit = *buf++;
        if ( (x < LLONG_MIN + digit)) {
            /*
              the decimal is bigger than any possible integer
              return border integer depending on the sign
            */
...
int decimal2lldiv_t(const decimal_t *from, lldiv_t *to) {
    ...
    if (int_part == 2)
        to->quot = ((long long)from->buf[0]) * DIG_BASE + from->buf[1];
    else if (int_part == 1)
        to->quot = from->buf[0];
    else
        to->quot = 0;
...
int double2lldiv_t(double nr, lldiv_t *lld) {
    ...
    lld->quot = (long long)(nr > 0 ? floor(nr) : ceil(nr));
    /* Multiply reminder to 10^9 and store into "rem" */
    lld->rem = (long long)rint((nr - (double)lld->quot) * 1000000000);
    /*
...
int decimal_round(const decimal_t *from, decimal_t *to, int scale,
    ...
    dec1 *buf0 = (dec1 *) from->buf, *buf1 = (dec1 *)to->buf;
    dec1 x, y, carry = 0;
    int first_dig;
...
int decimal_add(const decimal_t *from1, const decimal_t *from2, decimal_t *to) {
    if ( (from1->sign == from2->sign)) return do_add(from1, from2, to);
    return do_sub(from1, from2, to);
}

int decimal_sub(const decimal_t *from1, const decimal_t *from2, decimal_t *to) {
    if ( (from1->sign == from2->sign)) return do_sub(from1, from2, to);
    return do_add(from1, from2, to);
}

int decimal_cmp(const decimal_t *from1, const decimal_t *from2) {
    if ( (from1->sign == from2->sign)) return do_sub(from1, from2, 0);
...
int decimal_mul(const decimal_t *from_1, const decimal_t *from_2,
    ...
    */
    const decimal_t *from1 = from_1;
    const decimal_t *from2 = from_2;
    int intg1 = ROUND_UP(from1->intg), intg2 = ROUND_UP(from2->intg),
            frac1 = ROUND_UP(from1->frac), frac2 = ROUND_UP(from2->frac),
            intg0 = ROUND_UP(from1->intg + from2->intg), frac0 = frac1 + frac2, error,
            iii, jjj, d_to_move;
    dec1 *buf1 = (dec1 *)from1->buf + intg1, *buf2 = (dec1 *)from2->buf + intg2, *buf0, *start2,
    ...
    if ( (error)) {
        ...
        if ( (iii > intg0)) /* bounded integer-part */
        {
            iii -= intg0;
            jjj = iii >> 1;
            intg1 -= jjj;
...
static int do_div_mod(const decimal_t *from1, const decimal_t *from2,
    ...
    dec1 *buf0, *buf1 = (dec1 *)from1->buf, *buf2 = (dec1 *)from2->buf, *start1, *stop1, *start2,
    ...
    if (!(tmp1 = (dec1 *) malloc(len1 * sizeof(dec1)))) return E_DEC_OOM;
    ...
    */
    ...
    if ( (len2 > 0)) norm2 += (dec1)(norm_factor * start2[1] / DIG_BASE);
    ...
    for (; buf0 < stop0; buf0++) {
        ...
        if ( (dcarry == 0 && *start1 < *start2))
            ...
            if ( (guess >= DIG_BASE)) guess = DIG_BASE - 1;
            if ( (len2 > 0)) {
